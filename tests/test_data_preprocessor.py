...
Tests tokenization, class balancing, and data augmentation functionality.
"""
...
class TestDataPreprocessor:
    ...
    def test_tokenize_basic(self):
        """Test basic tokenization with CodeBERT tokenizer."""
        ...
        encoded = preprocessor.tokenize(texts, max_length=512)
    ...
    def test_tokenize_max_length_constraint(self):
        """Test that tokenization respects max_length of 512."""
        preprocessor = DataPreprocessor()
        
        # Create a very long text
        long_text = "x = 'secret' " * 1000  # Repeat to exceed 512 tokens
        
        encoded = preprocessor.tokenize([long_text], max_length=512)
        
        # Verify length is capped at 512
        assert encoded['input_ids'].shape[1] <= 512
    
    def test_tokenize_empty_list(self):
        """Test that tokenizing empty list raises error."""
        preprocessor = DataPreprocessor()
        
        with pytest.raises(PreprocessingError, match="Cannot tokenize empty text list"):
